...
fakenews = pd.DataFrame({'TITLE': fakenews['TITLE'], 'FAKE': np.ones([fakenews.shape[0]])})
ucinews = pd.DataFrame({'TITLE': ucinews['TITLE'], 'FAKE': ucinews['FAKE']})

# Conjoin them into one DataFrame, and we have ourselves a dataset to work with
conjoinedDataset = ucinews.append(fakenews)
